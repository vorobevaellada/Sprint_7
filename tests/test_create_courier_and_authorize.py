...
class TestsCreateNewCourier:
    ...
    def test_create_new_courier(self, generate_random_courier):
        ...
        with allure.step("Проверяем, что курьер успешно создан (статус 201 и ответ {'ok': True})"):
    ...
    def test_cannot_create_courier_without_login_or_password(self, data_setup):
        with allure.step("Отправляем POST-запрос на создание курьера с неполными данными"):
            response = requests.post(
                f"{ApiUrls.MAIN_URL}{ApiUrls.CREATE_COURIER}",
                json=data_setup
            )
        with allure.step("Проверяем ошибку 400 и сообщение \"Недостаточно данных...\""):  
            assert (
    ...
    def test_cannot_create_duplicate_courier(self, register_and_authenticate_courier):
        with allure.step("Отправляем POST-запрос на создание курьера с существующими данными"):
            response = requests.post(
                f"{ApiUrls.MAIN_URL}{ApiUrls.CREATE_COURIER}",
                json=register_and_authenticate_courier[0]
            )
        with allure.step("Проверяем ошибку 409 и сообщение \"Этот логин уже используется...\""):
            assert (
...
class TestLoginCourier:
    ...
    def test_courier_authentication_success(self, register_and_authenticate_courier):
        ...
        with allure.step("Проверяем успешность авторизации (статус 200 и присутствие ID)"):
    ...
    def test_failed_courier_authentication(self):
        with allure.step("Формируем некорректные данные для авторизации"):
            login_data = {
                "login": fakes.generate_user_login(),
                "password": fakes.generate_random_password()
            }
        with allure.step("Отправляем POST-запрос на авторизацию несуществующего курьера"):
            response = requests.post(
                f"{ApiUrls.MAIN_URL}{ApiUrls.COURIER_AUTHENTICATE}",
                json=login_data
            )
        with allure.step("Проверяем ошибку 404 и сообщение \"Учетная запись не найдена\""):
    ...
    def test_login_only_with_username(self, register_and_authenticate_courier):
        with allure.step("Готовим данные для авторизации без пароля"):
            response_data = {
                "login": register_and_authenticate_courier[2],
                "password": ""
            }
        with allure.step("Отправляем POST-запрос на авторизацию без пароля"):
            response = requests.post(
                f"{ApiUrls.MAIN_URL}{ApiUrls.COURIER_AUTHENTICATE}",
                json=response_data
            )
        with allure.step("Проверяем ошибку 400 и сообщение \"Недостаточно данных для входа\""):
    ...
    def test_login_only_with_password(self, register_and_authenticate_courier):
        with allure.step("Готовим данные для авторизации без логина"):
            response_data = {
                "login": "",
                "password": register_and_authenticate_courier[3]
            }
        with allure.step("Отправляем POST-запрос на авторизацию без логина"):
            response = requests.post(
                f"{ApiUrls.MAIN_URL}{ApiUrls.COURIER_AUTHENTICATE}",
                json=response_data
            )
